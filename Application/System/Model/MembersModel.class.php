...
class MembersModel extends Model
{
    ...
    // 列表
    function getList($param, $filed='u.*',$pageNum = 10){
        $page = isset($param['page'])? $param['page']: 1;

        $condition = [];
        if(!empty($param['keyword'])){
            $condition['s.realname'] = array('like',"%".$param['keyword']."%");
        }
        $total = $this->alias('u')->where($condition)->count();
        $Page = new \Think\PageAjax($total, $pageNum);
        $data['show'] = $Page->show(null);
        $data['Arrlist'] = $this->alias('u')->field($filed)
