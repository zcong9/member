...
Class WechatController extends Controller
{
    ...
    {
        //查询店铺的公众号信息
        $wechat = M('wechat')->where('restaurant_id=%d',$_SESSION['restaurant_id'])->find();
    ...
    public function wechatSet()
    {
        ...
        $wechat = M('wechat');
        $exist = $wechat->where('restaurant_id=%d',$_SESSION['restaurant_id'])->find();

        if($exist){
            //数据存在的，需要修改
            $res = $wechat->where('restaurant_id=%d',$_SESSION['restaurant_id'])->data($data)->save();
            if($res){
                $return['code'] = 0;  //修改成功
            }else{
                $return['code'] = 1;
