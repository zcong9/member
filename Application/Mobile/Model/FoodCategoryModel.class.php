...
class FoodCategoryModel extends Model
{
    ...
    const TABLE_NAME = 'food_category';
    ...
    public function getFoodListByRestaurantId($restaurant_id, $field = '*')
    {
        //查询出对应店铺的上架的分类
        $display_category = $this->model->field($field)->where(['is_display' => 1, 'restaurant_id' => $restaurant_id])->order('sort asc')->select();

        $category_id_list = [];
        $category_list = [];
        foreach ($display_category as &$value) {
            $category_id_list[] = $value['category_id'];
            $category_list[$value['category_id']] = $value;
        }

        $food = (new FoodModel)->getFoodByCategoryId($category_id_list);

        $category_list = $this->groupingFoodByCategory($food, $category_list);
     ...
     * @return array
     */
    private function groupingFoodByCategory($food, $category_list)
    {
        $food_specification_model = new FoodSpecificationModel;

        $food_list = [];
        if ($food) {
            foreach ($food as $value) {
                $value['have_attribute'] = $food_specification_model->isHaveSpecification($value['food_restaurant_id']) ? 1 : 0 ;
                $food_list[$value['category_id']][] = $value;
            }
        }

        foreach ($category_list as $key => $value) {
            $category_list[$value['category_id']]['food_list'] = $food_list[$value['category_id']];
        }
...
//            $where['is_timing'] = 1;
//            //表结构调整修改
//            $food_categoryIdList =  $food_category->where($where)->field('category_id')->select();
...
//        }
    ...
    public function layzLoad($food_category_id)
    {
        ...
        //用于排序
        $sortArr = array();
        foreach ($arrlist as $key => $v1) {
            $sortArr[] = $v1['sort'];
        }
        array_multisort($sortArr, SORT_ASC, $arrlist);
        return $arrlist;
    }
}
