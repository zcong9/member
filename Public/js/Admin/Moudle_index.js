...
// 竖屏广告顶部小图预览与上传
function smalltoppreview(position,file) {
    var prevDiv = $(file).parent()[0];
    var statu = $(file).siblings('img').attr('src'); //获取广告位状态（空或已有广告）
    if ($(file).siblings('img').attr('data-img') == "add") {
        statu = "";
    ...
    $.ajax({
        type: 'post',
        url: '/index.php/admin/Moudle/uploadtopimg/position/'+position,
        data: formdata,
        cache: false,
        processData: false, // 不处理发送的数据，因为data值是Formdata对象，不需要对数据做处理
        contentType: false, // 不设置Content-type请求头
        success: function(data) {
            layer.msg(vm.langData.success[vm.lang]);
            if( position == 'advertisement_index_small_image_url') {
                $("#indextop").html(data);
            } else {
                $("#foodtop").html(data);
            }
...
//删除竖屏顶部小广告
function deltopadver(position,z) {
    layer.confirm('', {
        title: vm.langData.deleteConfirm[vm.lang],
        btn: [vm.langData.yes[vm.lang], vm.langData.cancel[vm.lang]]
    }, function(index) {
        $.ajax({
            type: "post",
            url: "/index.php/admin/Moudle/deltopadver/position/"+position,
            data: { "advertisement_id": z },
            success: function(data) {
				try{
					data = JSON.parse(data);
